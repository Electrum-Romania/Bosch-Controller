...
// perspective shift
cv::Mat warp_lane(const cv::Mat& src, Pdata* pdata, const Options* options) {
...
cv::Mat extract_white(const cv::Mat& src, Pdata* pdata, const Options* options) {
...
cv::Mat fill_gaps(const cv::Mat& src, Pdata* pdata, const Options* options) {
...
std::vector<cv::Point2f> slidingWindow(const cv::Mat& image, cv::Rect2f window, Pdata* pdata)
{
